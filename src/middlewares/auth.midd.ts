...
export default class AuthMiddleware {
  ...
  verifyAuth() {
    this.verify = async (req: any, resp: Response, next: NextFunction) => {
      ...
      if (!authToken) return resp.json({ msg: 'UnAuthorized Token', ok: false });
      ...
      try {
        ...
        return resp.json({ msg: 'UnAuthorized IToken', error, ok: false });
      }
    };
    return this.verify;
  }
  ...
  isAdminAuth() {
    this.isAdmin = async (req: any, resp: Response, next: NextFunction) => {
      ...
      return resp.json({ msg: 'UnAuthorized IToken', ok: false });
    };
    return this.isAdmin;
  }
}
